...
export const ProtectedRoute: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // 1. Authenticated in React state or localStorage
  const storedToken =
    typeof window !== 'undefined'
      ? localStorage.getItem('prepora_token') || localStorage.getItem('prepora_auth_token')
      : null;
  const isAuth = isAuthenticated || !!token || !!storedToken;

  if (!isAuth) {
    // 2. Check if OAuth callback params exist in URL or if OAuth processing is in flight
