...
export const DoubtCenter: React.FC = () => {
  ...
  const handleSolveWithAI = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    setTimeout(() => {
      const solution = aiDoubtSolver.solveDoubt(aiQuestion, aiSubject, aiChapter);
  ...
  const sampleQuestions = [
    { text: 'How does work done in an isothermal reversible expansion differ from adiabatic expansion?', sub: 'Chemistry' as SubjectName, chap: 'Thermodynamics' },
    { text: 'Explain the 7 obligate taxonomical hierarchy ranks and why diversity decreases from kingdom to species.', sub: 'Biology' as SubjectName, chap: 'The Living World' },
    { text: 'Derive maximum range and height in projectile motion for complimentary launch angles.', sub: 'Physics' as SubjectName, chap: 'Motion in a Plane' }
  ];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-50 text-purple-700 text-xs font-semibold mb-2">
            ...
            <span>AI Academic Mentorship & Instant Solver</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Doubt Center</h1>
          <p className="text-sm text-slate-500 mt-1">
            Get instant AI step-by-step conceptual derivations or consult human mentor clarifications.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          ...
          >
            <Zap className="w-3.5 h-3.5 mr-1" /> Instant AI Solver
          </Button>
          <Button
            ...
            className="text-xs font-bold"
          >
            <MessageSquare className="w-3.5 h-3.5 mr-1" /> Mentorship Doubts ({doubts.length})
          </Button>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <Card className="p-5 border-purple-200/80 shadow-md shadow-purple-500/5 bg-gradient-to-b from-white to-purple-50/20">
            <form onSubmit={handleSolveWithAI} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">
                  Your Problem Statement or Conceptual Question
                </label>
                <textarea
                  rows={3}
                  value={aiQuestion}
                  onChange={(e) => setAiQuestion(e.target.value)}
                  placeholder="Paste problem text, formula derivation doubt, or tricky conceptual query here..."
              ...
              <div className="space-y-1.5">
                <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1">
                  <Lightbulb className="w-3 h-3 text-amber-500" /> Try a high-yield question:
                </span>
                <div className="flex flex-wrap gap-1.5">
                  {sampleQuestions.map((sq, i) => (
                    ...
                    >
                      {sq.chap}: {sq.text.substring(0, 40)}...
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-2">
                <p className="text-[11px] text-slate-400">
                  Powered by Prepora Academic Engine • NCERT & JEE/NEET standard compliant
                </p>
                <Button
                  type="submit"
                  variant="primary"
                  disabled={isSolving || !aiQuestion.trim()}
                  className="bg-purple-600 hover:bg-purple-700 font-bold text-xs flex items-center gap-1.5 px-5 py-2.5"
                >
                  <Sparkles className="w-4 h-4" />
                  {isSolving ? 'Solving Step-by-Step...' : 'Solve with AI'}
                </Button>
              </div>
            </form>
          </Card>
          ...
          {currentSolution && (
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <Card className="p-6 border-purple-200 bg-white space-y-5">
                <div className="flex items-start justify-between gap-4 pb-4 border-b border-slate-100">
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="text-xs font-bold shrink-0 text-purple-700 border-purple-200 hover:bg-purple-50"
                  >
                    <BookmarkPlus className="w-4 h-4 mr-1 text-purple-600" />
                ...
                <div className="p-4 rounded-xl bg-purple-50/70 border border-purple-100 space-y-1">
                  <div className="flex items-center gap-1.5 text-xs font-bold text-purple-900 uppercase tracking-wide">
                    <Lightbulb className="w-4 h-4 text-purple-600" /> Core Principle / Law
                  </div>
                ...
                {currentSolution.keyFormula && (
                  <div className="p-3.5 rounded-xl bg-slate-900 text-white space-y-1">
                    ...
                    <code className="text-xs font-mono text-emerald-300 font-bold block">
                      {currentSolution.keyFormula}
                    </code>
                  </div>
                )}
