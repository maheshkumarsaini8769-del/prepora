...
export const Login: React.FC<{ defaultTab?: 'login' | 'register' | 'otp' }> = () => {
  ...
  const handleZenuxsLogin = async () => {
    ...
    // Hard safety timeout: button loading cannot hang indefinitely
    const safetyTimer = setTimeout(() => {
      setIsLoggingIn((current) => {
        if (current) {
          setError('Authentication server response timed out. Please click #2 — Zenuxs Auth to try again.');
          return false;
        }
        return false;
      });
    ...
    try {
      ...
      // Resilient claim extraction from token
      if (tokens && !userInfo) {
      ...
      if (tokens || userInfo) {
        ...
        if (res.success) {
          setSuccessMsg('Authenticated successfully with #2 Zenuxs Auth! Redirecting...');
          setTimeout(() => {
            navigate(redirectTo, { replace: true });
          }, 300);
        } else {
